...
import 'documentVerification.dart';

class DocumentImageCapture extends StatefulWidget {
  final String email,documentType;
  DocumentImageCapture(Key key,this.email,this.documentType);
  @override
  _DocumentImageCaptureState createState() => _DocumentImageCaptureState();
}

class _DocumentImageCaptureState extends State<DocumentImageCapture> {
  File _imageFile;
        ...
        body: ListView(
          children: <Widget>[
                ...
                child: IconButton(
                  ...
                  onPressed:() {
                    ...
                    Navigator.pushReplacement(context, MaterialPageRoute(
                      builder: (context){
                        return DocumentVerificationScreen(email: widget.email,);
                      }
                    ));
                  },
                ),
            ...
            if(_imageFile!=null) ...[
              ...
              Padding(
                padding: const EdgeInsets.all(8.0),
                child: Uploader(file:_imageFile,email: widget.email,documentType: widget.documentType,),
              ),
            ],
            if(_imageFile==null)
              Stack(
...
class Uploader extends StatefulWidget {
  ...
  Uploader({Key key,this.file, this.email, this.event, this.documentType}) : super(key:key);
  @override
...
class _UploaderState extends State<Uploader> {
  ...
  Future<void> startUpload() async {
    String filePath = '${widget.email}/${DateTime.now()}_${widget.documentType}.png';
    setState(() {
      _task = _storage.ref().child(filePath).putFile(widget.file);
    });
    var downUrl = await (await _task.onComplete).ref.getDownloadURL();
    var url = downUrl.toString();
      if(widget.documentType=="adhar"){
        setState(() {
          adharImgUrl=url.toString();
        });
      }
      else if(widget.documentType=="PAN"){
        setState(() {
          PANImgUrl=url.toString();
        });
      }
      else{
        setState(() {
          VoterImgUrl=url.toString();
        });
      }
  }
  @override
